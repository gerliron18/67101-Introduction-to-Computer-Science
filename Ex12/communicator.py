...
# DESCRIPTION: The class that runs the networking between
#              the players
#############################################################
import socket
...
class Communicator:
    ...
    def __init__(self, root, port, ip=None):
        ...
        :param server: true if the initializing_communicator is started
                       in server mode, otherwise false.
        """
        self.__root = root
        self.__port = port
        self.__ip = ip
        ...
        # This means this initializing_communicator is a server
        # initializing_communicator and is responsible for trying and
    ...
    def connect(self):
        ...
        if self.__socket:
            return
        try:
            # This is the server initializing_communicator, try
            # and accept connections.
            if self.__server_socket is not None:
                self.__socket, _ = self.__server_socket.accept()
                self.__socket.setblocking(0)
                self.__server_socket.close()
            # This is the client initializing_communicator,
            # try and connect (quickly).
            else:
